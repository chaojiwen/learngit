...
/* RTC 文件内部的变量. */
static RTC_TYPEDEF	RTC_Time;	
//RTC_TYPEDEF	RTC_Time;//zwc
...
static bool Check_RTC_Time(void)
{
	...
	if((RTC_Time.STREG.Years/16<10) && (RTC_Time.STREG.Years%16<10) && \
		(RTC_Time.STREG.Months/16<10) && (RTC_Time.STREG.Months%16<10) && \
		(RTC_Time.STREG.Days/16<10) && (RTC_Time.STREG.Days%16<10) && \
	  (RTC_Time.STREG.Hours/16<10) && (RTC_Time.STREG.Hours%16<10) && \
	  (RTC_Time.STREG.Minutes/16<10) && (RTC_Time.STREG.Minutes%16<10) && \
	  (RTC_Time.STREG.Seconds/16<10) && (RTC_Time.STREG.Seconds%16<10))
	{
		ret=true;
	}
 ...
 * 描述  ：从 RTC 中读取 当前时间 数据 先判断各个日期属性是否合法（年：17~35；月：01~12；日：01~31；时：00~23；分：00~59; 秒：00~59）
 * 输入  ：time：存放数据的 指针
 * 输出  ：无
 * *******************************************************************zwc***************/
...
//  GSM_QUE*   GSM_QUE_inf;//定义消息体队列
	
	/* 读取 RTC 的时间数据 */
	RTC_Read(RTC_Time.REGS,0x00,7);
	if(Check_RTC_Time()==false)
	{
		return;
	}
/****************************对读取的时钟内容进行判定***************************/	
	if(((RTC_Time.STREG.Years>=0x17)&&(RTC_Time.STREG.Years<=0x35))     \
		  &&((RTC_Time.STREG.Months>0x00)&&(RTC_Time.STREG.Months <0x13)) \
	    &&((RTC_Time.STREG.Days>0x00)&&(RTC_Time.STREG.Days<0x32))      \
...
void RTC_System_Clock_Calibration(Times system_time)
{
	...
	/* 判断更新到达时间. */
	if(updata_time == System_Clock_Time_Updata)
	{
		/* 设置更新RTC时钟 */
		RTC_System_Clock_Calibration_State_Reg = FALSE;
	}
