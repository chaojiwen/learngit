...
//!SMS存储器存储的短信数量
#define MAX_NUM_SMS_MEM						50
//! log_cmd.h
// 各种命令的最大长度，包括AT+CMGR输出的原始短信命令
#define MAX_LEN_CMD							160
// 发送长短信时UDH的固定长度
...
// 普通指令的超时时间
#define TO_GPRS_GPCMD						15
// 普通指令的超时时间(返回OK)
#define TO_GPRS_RT_OK						2
// 域名解析
#define TO_GPRS_DO_DNS						30
// 设置APN
#define TO_GPRS_SET_APN						2
// 搜索模块的时间
#define TO_GPRS_FIND_MODULE					60
// 建立GPRS连接
#define TO_GPRS_SETUP_CONNECTION			60
// 断开GPRS连接
...
#define MAX_LEN_GPRS_PACKET_SEND			1024
...
#define TEST_PHONE_NUMBER					"15067448280"//"18072868677
...
#define GPRS_ID0_IP							""	//101.70.109.173//"210.83.80.208"
#define GPRS_ID0_PORT						"2502"

// 应用服务器的连接参数
#define GPRS_ID1_DN							"zjuee.net"
#define GPRS_ID1_IP							""
#define GPRS_ID1_PORT						"2505"

// 星历/升级服务器的连接参数
#define GPRS_ID2_DN							"zjuee.net"
#define GPRS_ID2_IP							""
#define GPRS_ID2_PORT						"2601"
//! @}
/*============================ TYPES =========================================*/
//!
namespace periph_c
{
    ...
    {
    public:
        static const uint32_t TMR_DLAY = 1;
        static const uint32_t TMR_RATE = 10;// 1: 1s, 2: 500ms, 10: 100ms
        static finline void Send(char ch)
        {
            COM3.PutChar(ch);
        }
        static finline void Debug(char ch)
        {
            DebugOutChar(ch);
        }
		//! exp_period	预期的周期值,1000:1KHz,实际周期值=exp_period/TMR_RATE.
        static void TimoutInit(uint32_t exp_period)
        {
			const uint32_t TMR_FREQ = 3;//(3x 1kHz)
            // Min:1100(Hz)
            int tmr_frequency = TMR_FREQ * 1000;
            // time:Max = 0xFFFF (tmr_period)
            TIMER7.Config(tmr_frequency, TMR_FREQ * exp_period / TMR_RATE);
...
namespace device
{
    enum DEV_GSM_RSSI_Mode
    {
        GSM_RSSI_MODE_DBM					= 1,

        GSM_RSSI_MODE_NONE					= 0,
    };
	...
	enum DEV_GSM_RING_Mode
    {
        GSM_RING_MODE_CALL_SMS				= 1,

        GSM_RING_MODE_CALL					= 0,
    };
    enum DEV_GSM_SMS_Status
    {
        GSM_SMS_STA_REC_UNREAD				= 0,

        GSM_SMS_STA_REC_READ				= 1,

        GSM_SMS_STA_STO_UNSEND				= 2,

        GSM_SMS_STA_STO_SENT				= 3,
    ...
    class DEV_GSM_Struct
    {
        ...
        uint32_t				__SMS_READY;
        uint32_t				__DEV_StartupTime;
    public:
    ...
    private:
        //!
        finline void DEV_GSM_Delay(uint32_t time)
        {
            if (__DEV_Delay)
            {
                __DEV_Delay(time);
        ...
        finline uint32_t DEV_GSM_Timing(void)
        {
            if (__DEV_SysTick)
                return __DEV_SysTick();
            return 0;
        }
        finline bool WaitForSMSReady(uint32_t delta)
        {
            return DEV_GSM_Timing() > __DEV_StartupTime + delta;
        }
    public:
        int 	DEV_GSM_Init(Func_Delay func1, Func_Systick func2, int baudrate, int &err ,T_GPRS_CONNECTION* ip_com,_SCSStruct * state);
        void	DEV_GSM_Config_Data_Network(T_GPRS_CONNECTION* ip_com);
		...
		//
        void   	DEV_GSM_Set_IP_COM(T_GPRS_CONNECTION* ip_com[MAX_NUM_TCP_CONN]);
		
		    void Hex2Str( const char *sSrc,  char *sDest, int nSrcLen );//将整型数转化为字符串
        ...
        bool	DEV_FetchPattern(char *ptn, int to, int &pos, int &err);
		   /**************************ZWC**************************************/
		    bool	DEV_FetchPattern_1(char *ptn, int to, int &pos, int &err);
        ...
        bool	GSM_CheckSMSAck(void);
        bool	GSM_GetVender(char *vender, int len);
        bool	GSM_GetSWVersion(char *version, int len);
        bool 	GSM_GetIMEI(char *imei, int len);
		    bool 	GSM_GetSMSCenter(char *smsctr, int len);
        bool	GSM_GetState(int *state);
        bool 	GSM_GetICCID(char *iccid, int len);
        bool	GSM_GetRSSI(int *rssi, enum DEV_GSM_RSSI_Mode mode = GSM_RSSI_MODE_NONE);
        bool	GSM_GetCCLK(char *cclk, int len);
        bool	GSM_GetIP(char *ip, int len);
        bool	GSM_GetDNS(char *ip, int len_ip, char *dn);
        bool	GSM_ReadSMS(int id, char *sms, int len);
        bool	GSM_SetBaudRate(int br);
        bool	GSM_SetGPRSAPN(void);
/********************************************GSM_ZWC9.15**************************************/				
				bool	GSM_CheckSIM_Carriers(void);//查询运营商名字
			  bool  GSM_CheckSIM_Signal_Strength(void);//查询SIM卡的信号强度
				bool  GSM_Set_SIM_APN(void);//设置SIM卡的APN (CMNET)
				bool  GSM_Set_SIM_Wireless_Link(void);//设置SIM卡的无线链路
				bool  GSM_Check_SIM_Local_IP(void);//获得本地IP地址
				bool	GSM_Close_Move_Scenario(void);//关闭移动场景 
				bool	GSM_Adhere_Business(void);//附着GPRS业务
				bool	GSM_Connection_Mode(void);//设置为GPRS连接模式
			  bool	GSM_Close_Tcp_Send_Data(void);//关闭GPRS数据连接
				bool	GSM_GPRS_Connection(void);//进行GPRS连接
				bool  GSM_Open_GPRS_Link(void);//打开GPRS连接
				bool	GSM_Set_Data_Show_Ip(void);//设置在接收数据前加IP头
				int 	DEV_Adaptive_BaudRate(void);//自适应波特率 */
				
				bool  DEV_SendAT_Cmd(char *at, char *ptn, int to, int &pos, int &err, bool ok = false);//发送AT指令
				bool  DEV_Send_GPRSAT_Cmd(char *at, char *ptn, int to, int &pos, int &err, bool ok = false);//发送AT指令 建立TCP连接   
      	bool  DEV_Send_Data_GPRSAT_Cmd(char *at, char *ptn, int to, int &pos, int &err, bool ok = false);//发送AT指令 发送数据
       	
	      bool	GSM_SendSMS(char* pdu, int len, int &err);
				
        bool	GSM_SetRingPinMode(enum DEV_GSM_RING_Mode mode);
        bool	GSM_SetSMSMode(enum DEV_GSM_SMS_Mode mode);
        bool	GSM_SetSMSMemMode(int addr = 0);
        bool	GSM_ListSMS(int *no, enum DEV_GSM_SMS_Status sta = GSM_SMS_STA_ALL);
        bool	GSM_DeleteSMS(int &no, int &err);
        ...
        bool	GSM_SendSMS(char *pn, char *ascii_utf8, int &err);
				bool	GSM_SendSMS1(char* pdu,char* PhoneNumUnicode, int len);
				
        bool	GSM_RecvSMS(char *pn, char* time, char *sms, int len, int &err);
        bool	GSM_ConnnectServerExample(char *data, int len, char *recv, int &rlen, int &err,
                                          enum DEV_GPRS_Mode mode = GSM_GPRS_MODE_TCP, int id = GPRS_ID0);
    };
}
/*============================ INTERFACE =====================================*/
